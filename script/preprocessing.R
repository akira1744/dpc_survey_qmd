...
# shiny用のdbを作成
con_qmd = dbConnect(SQLite(), 'data/dpc_survey_qmd.sqlite', synchronous="off")
...
# 医療法人名削除
mst_latest <- mst_latest %>% 
  mutate(hpname2 = str_replace(hpname,rmpattern,''),.after=hpname) %>%
  print()
...
create_agg_hp_all <- function(agg_long_hp,mst_latest){
  ...
  # 県の実績と各病院の実績を結合
  df_wide <- rbind(df_wide,ken_wide) %>% 
    print()
  ...
  df <- df %>% 
    ...
    rename(病院=hpname,医療圏=iryo,市=city,病床=dpcbed) %>% 
    print()
  
  # 合計列について、マスタ列を整理
  df <- df %>% 
    mutate(医療圏 = if_else(mstno=='0','埼玉県',医療圏)) %>% 
    mutate(病院 = if_else(mstno=='0', '埼玉県合計',病院)) %>% 
    mutate(mstno= if_else(mstno=='0',NA_character_,mstno)) %>%
    print()
  
  df <- df %>% 
    select(mstno,医療圏,市,病院,`2018`,`2019`,`2020`,`2021`,
           県内ランク= rank,
