...
create_hp_gt <- function(con,set_hp,table_row){
  ...
  hp <- hp %>% 
    filter(全国ランク <= table_row |  病院 %in% c('全国合計',set_hp)) 
...
create_agg_myhp_mdc2_all <- function(con,set_hp){
  ...
  agg_mdc2_all <- DBI::dbReadTable(con, 'agg_mdc2_all') %>% tibble() %>% print()
  ...
  df <- agg_mdc2_all %>% 
    filter(病院==set_hp) %>% 
    select(-医療圏,-市,-病院,-病床,-県) %>% 
    arrange(desc(`2021`)) %>% 
    rename(全国ランク=No) %>% 
    relocate(全国ランク,.after=`2021`) 
  ...
  df_tail_sum <- df_tail_sum %>% 
    select("MDC2", "疾患分類", "2018", "2019", "2020", "2021", "全国ランク", 
      "医療圏内シェア", "救急医療", "予定外")
...
create_agg_mdc6 <- function(agg_long_mdc6,table_row){
  ...
  agg_mdc6 <- agg_wide_mdc6 %>%
    ...
    inner_join(mst_latest,by='mstno') %>%
    select(-mstno) %>%
    select(mdc6cd,県,医療圏,市,病院,everything()) %>%
    rename(医療圏内シェア = pref_iryo_share) %>%
    rename(平均日数 = stay)
...
create_agg_long_mdc6_sum <- function(agg_long_mdc6,max_nend,set_pref){
  ...
  out <- agg %>% 
    ...
    mutate(県 = set_pref) %>%
    mutate(医療圏=NA_character_) %>% 
    mutate(市='') %>% 
    mutate(病院='合計') %>% 
    mutate(医療圏内シェア=NA_real_) %>%
    rename(平均日数=stay) %>% 
    mutate(病床 = NA_real_) %>%
    select(No,mdc6cd,県,医療圏,市,病院,`2018`,`2019`,`2020`,`2021`,医療圏内シェア,平均日数,病床) 
...
create_agg_mdc10 <- function(agg_long_mdc10,table_row){
  ...
  agg <- agg_wide %>%
    ...
    inner_join(mst_latest,by='mstno') %>%
    select(-mstno) %>%
    select(mdc6cd,opecd,県,医療圏,市,病院,everything()) %>%
    rename(医療圏内シェア = pref_iryo_share) %>%
    rename(平均日数 = stay)
...
create_agg_mdc10_sum <- function(agg_long_mdc10,max_nend,set_pref){
  ...
  out <- agg %>% 
    ...
    rename(平均日数=stay) %>% 
    mutate(病床 = NA_real_) %>%
    select(No,mdc6cd,opecd,県,医療圏,市,病院,`2018`,`2019`,`2020`,`2021`,医療圏内シェア,平均日数,病床) 
